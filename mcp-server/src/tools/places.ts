...
export interface Place {
  id: string;            // 'osm-123456' or 'curated-risonare'
  name: string;
  name_ja?: string;
  type: 'hotel' | 'guesthouse' | 'hostel' | 'pension' | 'ryokan' | 'campsite' | 'restaurant' | 'cafe' | 'bar';
  lat?: number;
  lon?: number;
  address?: string;
  phone?: string;
  ...
  must_try?: string;
  description?: string;
  tags_raw?: Record<string, string>;
  source: 'osm' | 'curated';
}

function osmTypeToPlaceType(tags: Record<string, string>): Place['type'] | null {
  const t = tags.tourism;
  const a = tags.amenity;
  if (t === 'hotel') return 'hotel';
  if (t === 'guest_house') return 'guesthouse';
  if (t === 'hostel') return 'hostel';
  if (t === 'motel' || t === 'pension') return 'pension';
  ...
  if ((t === 'hotel' || t === 'guest_house') && tags.name?.includes('旅館')) return 'ryokan';
  return null;
}

async function fetchOsmPlaces(placeType: 'accommodation' | 'food'): Promise<Place[]> {
...
const CURATED_ACCOMMODATION: Place[] = [
  {
    id: 'curated-risonare',
...
const CURATED_RESTAURANTS: Place[] = [
  {
    id: 'curated-toramaru',
    name: 'Toramaru Soba',
    name_ja: 'とらまる蕎麦',
...
export async function getAccommodation(args: {
  type?: 'hotel' | 'guesthouse' | 'pension' | 'ryokan' | 'campsite' | 'all';
  include_osm?: boolean;
  limit?: number;
}): Promise<Place[]> {
  ...
  let places: Place[] = [...CURATED_ACCOMMODATION];
  ...
  if (args.include_osm !== false) {
    try {
      ...
      const newOsm = osmPlaces.filter(op => {
        if (!op.lat || !op.lon) return false;
        return !CURATED_ACCOMMODATION.some(cp => {
          if (!cp.lat || !cp.lon) return false;
...
export async function getRestaurants(args: {
  ...
  const limit = args.limit ?? 20;
  let places: Place[] = [...CURATED_RESTAURANTS];
